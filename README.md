...
| 主从切换 | 从站模式无超时，可能阻塞主站轮询 | 增加 50ms 从站监听超时 + 50ms 冷却期防抖动 |
...
| UART 热更新 | 从站处理中直接重配 UART | 延迟至主站 IDLE 时安全执行 |
| EEPROM 磨损 | 每次写入都刷 EEPROM | 变更检测，数据未变则跳过写入 |
| 主站重试 | 超时后直接跳过 | 自动重试 2 次再放弃 |
...
1. **EEPROM 寿命**: 10 万次写入。内置变更检测保护——数据未变化时自动跳过写入。批量配置用 0x10 一次写入，避免反复 0x06。
2. **名称编码**: 名称字段按原始字节存储，支持 ASCII/GBK/UTF-8（由上位机决定编码）。
3. **RAM 用量**: sizeof(SystemCfg_t) ≈ 1332 字节，STM32L051 有 8KB RAM，足够。
4. **主从复用**: UART2 主从共用，主站 IDLE 间隙自动切入从站监听（50ms 超时自动切回 + 50ms 冷却期防抖动）。
5. **编译要求**: 需要 C11 支持（`_Static_assert`），GCC/ARMCC 默认支持。
6. **UART 热更新**: 通过 Modbus 写入 UART2 参数后，不会立即重配——等待主站空闲时自动执行，避免中断正在进行的通信。
7. **主站重试**: 单个数据点超时后自动重试 2 次（可通过 MB_MASTER_RETRY_MAX 调整），减少因网络抖动导致的数据丢失。
