# STM32L051 Modbus RTU 远程数据采集上报系统 v2.4
...
- **低功耗模式**: Standby 模式 ~0.4μA，RTC WakeUp 定时唤醒，休眠间隔可配置 (0~65535秒)
...
| 项目 | v2.3 | v2.4 |
|------|------|------|
| **低功耗模式** | Stop 模式 ~3μA, 唤醒后需恢复时钟 | **Standby 模式 ~0.4μA**, 唤醒后自动复位 |
| **唤醒流程** | Stop → 恢复时钟 → 恢复外设 → 继续 | **Standby → MCU 复位 → EEPROM 加载 → 继续** |
| **volatile 修复** | `g_run_mode` 缺少 volatile, 编译器可能优化掉 | **添加 volatile**, 确保中断/主循环间可见性 |
| **EEPROM 栈溢出** | `EEPROM_Save_Config` 栈上分配 ~1380 字节 | **逐字节直接比较**, 零额外栈消耗 |
| **帧间静默** | 发送后立即切接收, 可能收到回波 | **4ms 帧间静默** (Modbus RTU 3.5字符 @9600) |
| **名称寄存器检查** | 遍历 5×8=40 个区域检查 | **直接范围判断**, 提前退出 |
| **HEX 效率** | 循环中每次调用 strlen | **缓存字符串长度** |
...
- **Standby 模式**: 唤醒后 MCU 复位，RTC 和备份寄存器保持
...
15. **低功耗模式**: 寄存器 0x0018 配置休眠间隔（秒），设为 0 禁用。启用后，每轮轮询+上报完成自动进入 Standby 模式（~0.4μA），由 RTC WakeUp 定时唤醒。唤醒后 MCU 自动复位，从 EEPROM 加载配置后继续下一轮轮询。**注意**：配置模式（PA15=HIGH）下不进入低功耗。需要 CubeMX 配置 LSE (32.768kHz) 晶振。
16. **CubeMX 集成**: 需要在 CubeMX 中配置 RTC 时钟源为 LSE。Standby 模式唤醒后 MCU 自动复位，无需手动恢复时钟。
